...
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), 'duooffice-save-dir-'))
})
...
describe('resolveDefaultSaveDir', () => {
  ...
  it('creates and returns the fallback when nothing is configured', () => {
    const fallback = join(root, 'Documents', 'duoOffice')
...
describe('configuredDefaultSaveDir', () => {
  ...
  it('falls back to <Documents>/duoOffice without a setting', () => {
    const userData = join(root, 'userData')
    const documents = join(root, 'Documents')
    mkdirSync(userData, { recursive: true })
    const app = {
    ...
    expect(configuredDefaultSaveDir(app)).toBe(join(documents, 'duoOffice'))
    expect(existsSync(join(documents, 'duoOffice'))).toBe(true)
  })
})
