...
import type { DocGrid } from '@duooffice/docx-engine'
...
const KO_FONT_RE =
  /malgun|맑은|batang|바탕|myeongjo|myungjo|명조|gungsuh|궁서|gulim|굴림|dotum|돋움|nanum|나눔|duooffice (sans|serif) kr|(noto|source han) (sans|serif)[^,]*\bk(r|orean)?\b/i
...
export function cssFontFamily(font: string, followAltName = true): string {
  ...
  // 'duoOffice Box Drawing' (fonts.css): U+2500 rules would otherwise fall to
  // the fullwidth CJK subset and overflow the column
  const BOX = 'duoOffice Box Drawing'
  ...
  // 'duoOffice PUA Blank' keeps AI-residue PUA tokens invisible like Word
  // (Calibri/Carlito would otherwise supply a box .notdef for them).
  if (f.includes('aptos')) {
    return `${chain(font, 'Calibri', 'Carlito GO', 'duoOffice PUA Blank', CJK_SANS)},sans-serif`
  }
  ...
  // 'duoOffice Songti SC' (fonts.css local() alias of Songti SC): macOS Chromium
  ...
  if (f.includes('simsun') || f.includes('宋体') || f.includes('nsimsun')) {
    return `${chain(font, 'duoOffice Songti SC', 'STSong', 'SimSun', CJK_SERIF)},serif`
  }
  ...
  // 'duoOffice *' entries are CJK-only local() aliases (fonts.css): the underlying
  // system faces draw Cyrillic/Greek fullwidth, so those scripts must pass through
  const JA_SANS = ['Yu Gothic', 'duoOffice Hiragino Sans', 'Meiryo', 'Noto Sans JP']
  const JA_SERIF = [
    'Yu Mincho',
    'duoOffice Hiragino Mincho',
    'duoOffice MS Mincho',
    'Noto Serif JP',
  ]
  const KO_SANS = ['Malgun Gothic', 'duoOffice Sans KR', 'Apple SD Gothic Neo', 'Noto Sans KR']
  const KO_SERIF = ['duoOffice Batang', 'duoOffice Serif KR', 'duoOffice Myungjo', 'Noto Serif KR']
  const TC_SANS = ['Microsoft JhengHei', 'PingFang TC', 'duoOffice Heiti TC', 'Noto Sans TC']
  // 'duoOffice Fullwidth TC' (fonts.css): fullwidth U+FF0D/FF0F/FF3C/FF3F/FF5E whose Songti TC glyphs look half-width
  const TC_SERIF = ['duoOffice MingLiU', 'duoOffice Fullwidth TC', 'Songti TC', 'Noto Serif TC']
  const SC_SANS = ['PingFang SC', 'Microsoft YaHei', CJK_SANS]
  const SC_SERIF = ['duoOffice Songti SC', 'STSong', 'SimSun', CJK_SERIF]
  const nfkc = font.normalize('NFKC')
  ...
  if (
    ...
    if (/nanum ?gothic|나눔 ?고딕/i.test(nfkc)) {
      return `${chain(font, 'duoOffice Gothic KR', ...KO_SANS)},sans-serif`
    }
    ...
    if (/(?:batang|gulim|dotum|gungsuh) ?che\b|(?:바탕|굴림|돋움|궁서)체/i.test(nfkc)) {
      ...
      return `${chain(font, 'duoOffice Che Latin KR', ...(cheGungsuh ? ['GungSeo'] : []), ...(cheSerif ? KO_SERIF : KO_SANS))},${cheSerif ? 'serif' : 'sans-serif'}`
    }
  ...
  if (/nyala|ebrima|abyssinica|ethiopic/i.test(nfkc)) {
    return `${chain(font, 'duoOffice Ethiopic')},sans-serif`
  }
  ...
  if (/tamil|latha|vijaya|inaimathi/i.test(nfkc)) {
    return `${chain(font, 'duoOffice Tamil', 'InaiMathi', 'Tamil MN', 'Tamil Sangam MN')},sans-serif`
  }
  ...
  const pua = latinNamed ? ['duoOffice PUA Blank'] : []
